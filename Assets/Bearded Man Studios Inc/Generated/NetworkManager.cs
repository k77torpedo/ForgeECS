...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] ECSNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is ECSNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (ECSNetworkObject.Length > 0 && ECSNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(ECSNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<ECSBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateECS instead, its shorter and easier to type out ;)")]
		public ECSBehavior InstantiateECSNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(ECSNetworkObject[index]);
			var netBehavior = go.GetComponent<ECSBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<ECSBehavior>().networkObject = (ECSNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of ECS
		/// </summary>
		/// <returns>
		/// A local instance of ECSBehavior
		/// </returns>
		/// <param name="index">The index of the ECS prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public ECSBehavior InstantiateECS(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (ECSNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: ECS. Add them at the NetworkManager prefab.");
				return null;
			}
